...
// | @Company: EfarSoft
// +----------------------------------------------------------------------
// | @Copyright: © 2020~2028 http://efarsoft.cn All rights reserved.
// +----------------------------------------------------------------------
// | @Author: EfarSoft Team <efarhost@foxmail.com>,<405146868@qq.com>
// +----------------------------------------------------------------------
// | @Date: 2021-09-01 15:26:29
// +----------------------------------------------------------------------
// | @Description: Ulid For PHP
// +----------------------------------------------------------------------

namespace Efarsoft;

use Efarsoft\Exception\InvalidUlidException;
...
{   
    ...
    public static function fromString(string $value, bool $lowercase = false): self
    {
        if (strlen($value) !== static::TIME_LENGTH + static::RANDOM_LENGTH) {
            throw new InvalidUlidException('Invalid ULID string (wrong length): ' . $value);
        }
        ...
        if (!preg_match(sprintf('!^[%s]{%d}$!', static::ENCODING_CHARS, static::TIME_LENGTH + static::RANDOM_LENGTH), $value)) {
            throw new InvalidUlidException('Invalid ULID string (wrong characters): ' . $value);
        }
    ...
    private function decodeTime(string $time): int
    {
        ...
        foreach ($timeChars as $index => $char) {
            if (($encodingIndex = strripos(static::ENCODING_CHARS, $char)) === false) {
                throw new InvalidUlidException('Invalid ULID character: ' . $char);
            }
        ...
        if ($carry > static::TIME_MAX) {
            throw new InvalidUlidException('Invalid ULID string: timestamp too large');
        }
